...
- Auth can be interactive or via TS\_AUTHKEY
- TS\_FORCE\_LOGIN=1 forces fresh registration
  ...
  <Accordion title="Common Failure Modes">
    <AccordionGroup>
      <Accordion title="A) Client never connects">
        ...
        - Enable debug
        - Or use TS\_AUTHKEY \+ TS\_FORCE\_LOGIN=1
      </Accordion>
...
On first run, MacOS may indicate that the binary is dangerous. To resolve this, after you have chmod and executed the file you can use `cmd + space` to go to Privacy and Security, scroll down to the _Security_ section and click `Allow Anyway` next to the name of the bin. On the subsequent execution you will be asked to Open the file in a prompt. Click `Open Anyway, ` type in your password on the next screen, and you should be free to operate the binary.